...
- **Testing Mandate**: Ensure all new features or significant bug fixes include corresponding RSpec tests. Aim to maintain or increase test coverage. Do not commit changes until all tests pass. Run tests as needed to confirm the correctness of a change. Do not hesitate to add additional tests.
...
- **Progress Indication & Logging**:
  ...
  - **Enhanced Contextual Logging**: Improved logger to include structured context information along with log messages, making debugging and monitoring easier. Logs now include relevant information like operation counts, durations, and identifiers with each log entry.
  - **Debug Mode**: Added dedicated debug logging level that only appears when LINEAR_CLI_DEBUG=true is set.
